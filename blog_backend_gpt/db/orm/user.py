...
class UserSession(Base):
    ...
    session_token = mapped_column(String(256), unique=True, name="sessionToken")
    user_id = mapped_column(
        String(256), ForeignKey("User.id", ondelete="CASCADE"), name="userId"
    )
    expires = mapped_column(DateTime)

    user = relationship("User")

    __table_args__ = (Index("user_id_index", "userId"),)
...
class User(Base):
    ...
    email = mapped_column(String(256), nullable=True, unique=True)
    email_verified = mapped_column(DateTime, nullable=True, name="emailVerified")
    image = mapped_column(String(256), nullable=True)
